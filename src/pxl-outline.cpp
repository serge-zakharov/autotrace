...
static pixel_outline_type find_one_outline (at_bitmap*, edge_type, unsigned short,
                                            unsigned short, at_bitmap *, bool, bool, at_exception_type *);
...
static bool is_marked_edge (edge_type, unsigned short, unsigned short, at_bitmap*);
static bool is_outline_edge (edge_type, at_bitmap*, unsigned short, unsigned short,
...
static bool is_marked_dir(unsigned short, unsigned short, direction_type, at_bitmap*);
static bool is_other_dir_marked(unsigned short, unsigned short, direction_type, at_bitmap*);
static void mark_dir(unsigned short, unsigned short, direction_type, at_bitmap *);
static bool next_unmarked_pixel(unsigned short *, unsigned short *,
                                   direction_type *, at_bitmap *, at_bitmap *);

bool is_valid_dir (unsigned short, unsigned short, direction_type, at_bitmap*, at_bitmap*);

static at_coord next_point(at_bitmap*, edge_type *, unsigned short *, unsigned short *, 
                           at_color, bool, at_bitmap *, at_exception_type * );
...
find_outline_pixels (at_bitmap * bitmap, at_color *bg_color,
		     at_progress_func notify_progress, void* progress_data,
		     at_testcancel_func test_cancel, void* testcancel_data,
  ...
  for (row = 0; row < AT_BITMAP_HEIGHT (bitmap); row++)
    {
      for (col = 0; col < AT_BITMAP_WIDTH (bitmap); col++)
        {
          ...
          if (notify_progress)
            notify_progress((float)(row * AT_BITMAP_WIDTH(bitmap) + col) / ((float) max_progress * (float)3.0),
                            progress_data);
	  ...
	  at_bitmap_get_color(bitmap, row, col, &color);
          if (!(is_background = (bool)(bg_color && at_color_equal(&color, bg_color)))
...
find_one_outline (at_bitmap * bitmap, edge_type original_edge,
                  ...
                  at_bitmap *marked, bool clockwise, bool ignore,
                  at_exception_type * exp)
{
  pixel_outline_type outline;
...
bool is_valid_dir (unsigned short row, unsigned short col, direction_type dir, at_bitmap * bitmap, at_bitmap * marked)
{
                 ...
                 at_exception_type * exp)
{
  ...
  switch (edge)
    ...
    case RIGHT:
      return (bool)(col == AT_BITMAP_WIDTH (bitmap) - 1
			|| !at_bitmap_equal_color(bitmap, row, col + 1, &color));

    case BOTTOM:
      return (bool)(row == AT_BITMAP_HEIGHT (bitmap) - 1
			|| !at_bitmap_equal_color(bitmap, row + 1, col, &color));
...
{
  return (bool)((*AT_BITMAP_PIXEL(marked, row, col) & 1 << dir) != 0);
}
...
{
  return (bool)((*AT_BITMAP_PIXEL(marked, row, col) & (255 - (1 << dir) - (1 << ((dir + 4) % 8))) ) != 0);
}
...
{
  return
    (bool)(edge == NO_EDGE ? FALSE : (*AT_BITMAP_PIXEL (marked, row, col) & (1 << edge)) != 0);
}

static at_coord next_point(at_bitmap* bitmap, edge_type *edge, unsigned short *row, unsigned short *col,
                           at_color color, bool clockwise, at_bitmap * marked,
                           at_exception_type * exp)
{
  at_coord pos = {0, 0};
