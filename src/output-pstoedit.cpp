...
static int output_pstoedit_writer (FILE* file, char* name,
...
static void remove_temporary_file (const char* tmpfile_name);
...
/* This output routine uses two temporary files to keep the
   ...
   -> file */
static int
output_pstoedit_writer (FILE* file, char* name,
			int llx, int lly, int urx, int ury, 
			at_output_opts_type * opts,
			at_spline_list_array_type shape,
			at_msg_func msg_func, 
...
bool
unusable_writer_p(const char* suffix)
{
...
static void
remove_temporary_file (const char* tmpfile_name)
{
#if BO_DEBUG == 0
  remove (tmpfile_name);
#else 
