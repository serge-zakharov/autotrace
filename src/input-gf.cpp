...
get_character_bitmap (gf_char_t *sym)
{
	...
	sym->height = height;
	sym->width = width;
	sym->bitmap = (char*) calloc (width, height);
	if (! sym->bitmap) {
...
deblank (gf_char_t *sym)
{
	...
	if (white_on_left > 0 || white_on_right > 0 ||
		...
		if (white_on_left + white_on_right > sym->width) {
			...
			condensed.bitmap = (char*) calloc (condensed.width,
				condensed.height);
			if (! condensed.bitmap) {
				fprintf (stderr, "%s: out of memory\n",
					sym->font->input_filename);
...
at_bitmap input_gf_reader (char* filename, at_input_opts_type *opts,
				at_msg_func msg_func, void* msg_data, void* user_data)
{
