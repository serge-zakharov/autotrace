...
#define OUT_REAL(r)	fprintf (pdf_file, r == (ROUND (r = ROUND((float)6.0*r)/(float)6.0))				\
                                  ? "%.0f " : "%.3f ", r)
...
#define SOUT_REAL(r)	sprintf (temp, r == (ROUND (r = ROUND((float)6.0*r)/(float)6.0))				\
...
static int output_pdf_header(FILE* pdf_file, char* name,
...
int output_pdf_writer(FILE* pdf_file, char* name,
		      int llx, int lly, int urx, int ury, 
