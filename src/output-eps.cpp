...
#define OUT_REAL(r)	fprintf (ps_file, r == (ROUND (r = ROUND((float)6.0*r)/(float)6.0))				\
                                  ? "%.0f " : "%.3f ", r)
...
static int output_eps_header(FILE* ps_file, char* name,
...
int output_eps_writer(FILE* ps_file, char* name,
		      int llx, int lly, int urx, int ury, 
