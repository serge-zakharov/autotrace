 ...
 * 0.0 <= percentage <= 1.0
 */
typedef void           (* at_progress_func)    (float percentage,
						void* client_data);
...
at_bitmap * at_bitmap_read (at_bitmap_reader * reader,
				 ...
				 at_msg_func msg_func, void* msg_data);
...
void at_bitmap_get_color (const at_bitmap * bitmap,
			  ...
			  at_color * color);
bool at_bitmap_equal_color(const at_bitmap * bitmap,
			       unsigned int row, unsigned int col,
			       at_color * color);
...
std::vector<at_string_tuple> at_input_list_new(void);
std::string at_input_shortlist (void); 
...
std::vector<at_string_tuple> at_output_list_new (void);
std::string at_output_shortlist (void); 
