...
#define OUT_REAL(r)	fprintf (epd_file, r == (ROUND (r = ROUND((float)6.0*r)/(float)6.0))				\
                                  ? "%.0f " : "%.3f ", r)
...
static int output_epd_header(FILE* epd_file, char* name,
...
int output_epd_writer(FILE* epd_file, char* name,
		      int llx, int lly, int urx, int ury, 
