...
at_color *
at_color_parse (const char * string)
{
  unsigned char c[6];
  int i;
  ...
  if (!string)
    ...
    {
		at_set_error(AT_ERROR, AT_ERROR_WRONG_COLOR_STRING, std::string("color string is too short: ") + string);
        return NULL;
    }
...
hctoi (char c)
{
  if (c == '0')
    ...
    {
		at_set_error(AT_ERROR, AT_ERROR_WRONG_COLOR_STRING, std::string("wrong char in color string: ") + c);
