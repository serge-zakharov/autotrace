...
#include "../autotrace/autotrace/stdafx.h"
#include "autotrace.h"
#include "message.h"
#include "cmdline.h"
#include "logreport.h"
  ...
  while (TRUE)
    {
      ...
      if (ARGUMENT_IS ("background-color"))
        {
			try{
				fitting_opts->background_color = at_color_parse(optarg);
      ...
      else if (ARGUMENT_IS ("debug-arch"))
	{
	  ...
	  char *ishortlist, *oshortlist;
          ...
          fprintf (stderr, USAGE2, at_input_shortlist().c_str(),  at_output_shortlist().c_str());
          fprintf (stderr, 
...
{
  auto list = at_output_list_new();
  for (auto &item : list)
	  fprintf(file, "%10s %s\n", item.first.c_str(), item.second.c_str());
}

static void
dot_printer(float percentage, void* client_data)
{
...
static void
exception_handler(const char* msg, at_msg_type type, void* data)
{
